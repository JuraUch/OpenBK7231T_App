...
extern "C" commandResult_t IR_Send_Cmd(const void *context, const char *cmd, const char *args_in, int cmdFlags) {
	...
	//Reference: C Strings 31: Split string by space into words [C Programming]
	//YouTube Channel: JLabs
	for (int i = 0; i <= strlen(args); i++){
		if(args[i] == '-' || args[i] == '\0'){
			...
			words[totalWords][wordIndex] = args[i];
			wordIndex++;
		}
	...
	strncpy(tmp_protocol, words[0], sizeof(tmp_protocol)-1);
    strncpy(tmp_data, words[1], sizeof(tmp_data)-1);

    uint64_t data = strtoull(tmp_data, NULL, 16);
	decode_type_t protocol = strToDecodeType(tmp_protocol);
	...
	if (pIRsend) {
		...
		switch(protocol)
		{
		 	...
		 	case decode_type_t::GREE:
				pIRsend->sendGree(data);
		 		break;
		 	case decode_type_t::KELON:
				pIRsend->sendKelon(data);
		 		break;
		 	default:
		...
		memset(tmp_protocol, '\0', sizeof(tmp_protocol));
		 
		ADDLOG_INFO(LOG_FEATURE_IR, (char *)"IR send...");
		return CMD_RES_OK;
	}
	else {
		ADDLOG_INFO(LOG_FEATURE_IR, (char *)"IR NOT send (no IRsend running)");
	}
	return CMD_RES_ERROR;
}
